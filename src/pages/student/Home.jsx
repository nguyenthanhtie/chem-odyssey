...
const BentoCard = ({ title, highlight, description, linkText, linkUrl, graphic, className = '' }) => (
  <motion.div variants={itemVariants} className={`group relative card-tactile p-8 flex flex-col gap-6 hover:-translate-y-1 overflow-hidden h-full ${className}`}>
    ...
    <div className="relative z-10 mt-auto">
      <Link to={linkUrl} className="inline-flex items-center gap-3 px-6 py-3 btn-tactile-green rounded-full w-max text-sm uppercase tracking-widest font-black">
        {linkText}
...
const Home = () => {
  ...
  return (
    <div className="min-h-screen font-sans bg-[oklch(0.98_0.02_135)] selection:bg-viet-green selection:text-white">
        ...
        <div className="max-w-[1200px] mx-auto px-6 relative z-10">
          ...
          >
            ...
            <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-6 items-center justify-center">
              <Link to="/classroom" className="btn-tactile-green text-xl font-black px-10 py-5 rounded-[1.5rem] whitespace-nowrap flex items-center gap-3">
                {t('home.enter_classroom')}
                <span className="text-2xl">⚡</span>
              </Link>
              <Link to="/lab" className="bg-white text-[#1a1a1a] border-2 border-duo-border border-b-4 hover:bg-gray-50 active:border-b-0 active:translate-y-[4px] transition-all text-xl font-black px-10 py-5 rounded-[1.5rem] whitespace-nowrap flex items-center gap-3">
                {t('home.enter_lab')}
                <span className="text-2xl">🧪</span>
              </Link>
            </motion.div>
      ...
      {/* --- BENTO GRID SECTIONS --- */}
      <section className="py-20 bg-[oklch(0.96_0.03_135)] border-t-2 border-b-2 border-duo-border">
        <div className="max-w-[1200px] mx-auto px-6">
          ...
          >
            {/* 1. Lớp học (Spans 2 columns wide) */}
            <BentoCard
