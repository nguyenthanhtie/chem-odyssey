      ...
      <div className="max-w-7xl mx-auto">
        <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-viet-text-light font-bold">Tra cứu lý thuyết và video bài giảng theo khối lớp</p>
          </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32">
            ...
            <p className="text-viet-text-light font-bold">Đang tải dữ liệu...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              >
                <div className="flex items-center justify-between mb-4">
                   <span className="text-[10px] font-black text-viet-green uppercase tracking-widest">
                      Bài {lesson.order || lesson.lessonId.split('_').pop()}
                   </span>
                ...
                <div className="flex items-center gap-4 text-[10px] font-bold text-viet-text-light/60 uppercase">
                   <span className="flex items-center gap-1">🎥 1 Video</span>
                   <span className="flex items-center gap-1">📖 Lý thuyết</span>
                </div>
              </Link>
            ))}
            ...
            {filteredLessons.length === 0 && (
              <div className="col-span-full text-center py-24 bg-white/50 rounded-[32px] border-2 border-dashed border-viet-border">
                <p className="text-viet-text-light font-bold text-lg">Không tìm thấy bài giảng nào khớp với từ khóa</p>
                <button onClick={() => setSearchQuery('')} className="mt-4 text-viet-green font-bold hover:underline">Xóa bộ lọc</button>
              </div>
            )}
          </div>
        )}
